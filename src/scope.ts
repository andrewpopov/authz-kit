 ...
 */
export function mapScopeRole<ParentRole extends string, ChildRole extends string>(
  parentRole: unknown,
  table: Partial<Record<ParentRole, ChildRole>>,
  options: { fallback?: ChildRole } = {},
): ChildRole | undefined {
  if (typeof parentRole !== 'string') return options.fallback;
  const mapped = table[parentRole as ParentRole];
