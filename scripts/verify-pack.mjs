 ...
 *
 * authz-kit has ZERO runtime dependencies, so the throwaway consumer
 * installs only the tarball.
 *
...
const workDir = mkdtempSync(join(tmpdir(), 'authz-kit-verify-'));
let tarballPath;
...
try {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: 'authz-kit-consumer', version: '1.0.0', private: true }, null, 2),
  );
