...
- Unknown/malformed account-admin mutations are denied, definePolicy/defineRoles reject fail-open definitions at definition time, and authorize()'s action lookup can no longer be reached through the prototype chain.
...
`require('@andrewpopov/authz-kit/package.json')` threw
